    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-4">
      {!publiclyRendered && (
        <p className="text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded px-3 py-2">
          This page is kept in the content data but is <strong>not shown on the public site</strong>
          {' '}(tips and Lazy-Daze pages were retired during the migration). Edits here update the
          data only.
        </p>
      )}
        ...
        <label htmlFor="page-published" className="text-sm text-gray-700">
          ...
          {!published && (
            <span className="block text-xs text-amber-700 mt-0.5">
              Draft: only you (logged in) can view this page at its URL; it stays out of
              search and the sitemap until published.
            </span>
          )}
        </label>
      </div>
      ...
      <div className="text-xs text-gray-400">
        Slug: <code>/{page.slug}</code> &middot; type: <code>{page.type}</code> (not editable) &middot; nid {page.nid}
      </div>
